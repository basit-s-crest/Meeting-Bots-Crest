...
export function LiveQAOverlay({
  ...
  return (
    <Card className={cn("flex flex-col p-5 shadow-sm border border-border bg-surface overflow-hidden min-h-0", className)}>
      ...
      <div className="mb-3 flex-1 min-h-0 space-y-3 overflow-y-auto pr-1">
        {qaHistory.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-2 py-8 text-center border border-dashed border-border rounded-xl bg-surface-2/30">
          ...
          qaHistory.map((item) => (
            <div
              key={item.id}
              className="space-y-2 rounded-xl border border-border bg-surface-2/40 p-3 transition-all duration-200 text-xs"
            >
              {/* Question */}
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-2">
                  <span className="mt-0.5 rounded bg-brand-100 px-1.5 py-0.5 text-[10px] font-bold text-brand-700">
                    Q
                  </span>
                  ...
                  <div>
                    <div className="text-xs leading-relaxed text-ink-soft">
                      {item.answer ? (
                        ...
                        <span className="text-[11px] italic text-ink-mute flex items-center gap-1.5">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-1.5 pt-1">
                        ...
                        {item.citations.map((cite, cIdx) => (
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-md border border-brand-200 bg-brand-50/80 px-1.5 py-0.5 text-[11px] font-semibold text-brand-700 hover:bg-brand-100 hover:border-brand-300 transition-all cursor-pointer shadow-2xs"
      ...
      {/* Question Input Form */}
      <form onSubmit={handleSubmit} className="mt-auto flex shrink-0 gap-2 border-t border-border pt-3">
        <input
          type="text"
          value={questionInput}
